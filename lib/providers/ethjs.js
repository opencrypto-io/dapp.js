...
class EthjsProvider extends Provider {
  ...
  async send (contract, method, opts, sendOpts) {
    //const txInfo = await this._api.getTransactionByHash(tx)
    /*const filter = contract.show_message_from_doner()
    filter.new()
    filter.watch((err, res) => {
      console.log(res)
    })*/
    const tx = await contract[method].apply(null, opts.concat([sendOpts]))
    return tx
  }
