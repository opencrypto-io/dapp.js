...
class Service {
  constructor (api, index, assets) {
    ...
    this._index = index
    this._assets = assets
    this.$debug = this._api._debug('Service:' + this.constructor.name)
    this.$utils = utils
  }
  ...
  async $msend (...args) {
    return this.$send(null, ...args)
  }

  async $events (...args) {
    return this._api.contractEvents(this, ...args)
  }

  async $mevents (...args) {
    return this.$events(null, ...args)
  }
}
